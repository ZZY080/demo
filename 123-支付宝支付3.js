...
app.get('/pay', async (req, res) => {
  try {
    ...
    formData.addField('bizContent', {
      // 消费者订单号,64个字符以内、可包含字母、数字、下划线,且不能重复  可以随便填
      outTradeNo: '29163636562sasa39@qq.com',
      // 销售产品码，与支付宝签约的产品码名称,仅支持FAST_INSTANT_TRADE_PAY
      productCode: 'FAST_INSTANT_TRADE_PAY',
      subject: '华为技术科技有限公司', // 订单标题
      body: '商品详情', // 订单描述
...
app.get('/api/paymentQuery', function (req, res, next) {
  ...
  const result = alipaySdk
    .exec('alipay.trade.query', {}, { formData: formData })
